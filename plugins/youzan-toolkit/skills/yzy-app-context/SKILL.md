...
脚本入口同时提供 PowerShell、Python 和 Bash 三种形式。Windows 原生环境优先使用 PowerShell；通用环境优先尝试 Python；如果当前环境没有可用 PowerShell/Python 但有 Bash，再尝试 Bash 入口。
...
```bash
powershell -ExecutionPolicy Bypass -File <plugin-root>/tools/zancli/ensure_zancli.ps1 -- zancli app context --output json
# or:
...
```bash
powershell -ExecutionPolicy Bypass -File <plugin-root>/tools/zancli/ensure_zancli.ps1 -- zancli app context --app-id 79782 --env dev --output json
powershell -ExecutionPolicy Bypass -File <plugin-root>/tools/zancli/ensure_zancli.ps1 -- zancli app context --app-name self-container-test --env dev --output json
# or:
...
- 日志、链路追踪与只读查询：上下文解析成功后，向实际 `zancli` 命令传入相同的 `--app-id` 或 `--app-name`、`--env`，防止当前目录变化导致目标漂移。
