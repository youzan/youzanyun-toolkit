---
name: yzy-rds
description: 查询和操作有赞应用绑定的 RDS 数据库。用于查库、查表、查表结构、DDL 预检查、DML 查询与历史查看时；所有应用级操作前必须先解析应用上下文并使用 内部 CLI 工具校验登录。不能直接操作线上数据库。
---

# YZY RDS

所有 `rds` 操作都先使用 `yzy-app-context` 解析目标应用；该 skill 会连带完成 内部 CLI 工具的安装和登录校验。不要直接根据当前目录、应用名或上一轮对话执行数据库命令。
