---
name: zancli-api-capability
description: 查询和预检有赞云开放 API 能力包授权。用于设计或调用有赞云开放 API 前，确认当前应用类目下的 API 能力包、能力包详情、目标 API 是否已授权；所有应用级 capability 操作前必须先解析应用上下文并使用 zancli-bootstrap 校验登录。
---
...
所有 `capability` 操作都先使用 `zancli-app-context` 解析目标应用；该 skill 会连带完成 `zancli-bootstrap` 的安装和登录校验。不要直接根据当前目录、应用名或上一轮对话执行 API 能力包命令。
